...
class GenericBrowserAdapter(BaseAdapter):
    ...
    def crawl(self, url: str, options: CrawlOptions) -> CrawlResult:
        ...
        try:
            ...
            raise RuntimeError("Playwright is required for unknown dynamic sites: pip install '.[browser]' && playwright install chromium") from exc
        ...
        def absorb_json(data: object, base_url: str) -> None:
            for obj in walk_dicts(data):
                ...
                key = jid or job_url
                jobs[key] = Job(
                    id=jid or key, title=title, url=job_url, location=clean_text(find_first(obj, LOC_KEYS)),
                    description=clean_text(find_first(obj, DESC_KEYS)), requirements=clean_text(find_first(obj, REQ_KEYS)),
                    source=host, extra=obj,
                )
        ...
        with sync_playwright() as p:
            ...
            page.wait_for_timeout(options.browser_wait_ms)
            for _ in range(min(options.max_pages, 20)):
                ...
                if len(jobs) >= options.max_jobs:
                    break
            # DOM anchors as a final fallback
        ...
        # clean duplicate titles + urls generated by recursive JSON traversal
