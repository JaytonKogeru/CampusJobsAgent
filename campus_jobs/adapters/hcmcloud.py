...
class HCMCloudAdapter(BaseAdapter):
    """HCMCloud public recruiting portals.

    HCMCloud can wrap recruiting API requests/responses in its browser-side `ha5`
    transfer envelope. Rather than depending on that private wire format, this
    adapter lets the official frontend decrypt/render the public list, then reads
    normalized job links from the DOM and pages the official UI.
    """
    ...
    def _candidate_to_job(candidate: dict, base_url: str, host: str) -> Job | None:
        ...
        href = canonical_url(raw_href, base_url)
        if not href:
            return None
        low_href = href.lower()
        if not any(token in low_href for token in ("job_detail", "jobdetail", "position_detail", "portal_job_detail")):
            return None
        card_text = clean_text(candidate.get("card_text"))
        title = HCMCloudAdapter._title_from_text(candidate.get("title") or card_text)
        if not title or title in _BAD_TITLES:
            return None
        match = re.search(r"[?&](?:id|job_id|position_id)=([^&#]+)", href, re.I)
        jid = match.group(1) if match else href
        return Job(
            id=jid,
            title=title,
            url=href,
            location=HCMCloudAdapter._location_from_text(card_text),
            source=host,
            extra={"list_text": card_text[:1500]},
        )
    ...
    def _href_sample(page, limit: int = 30) -> list[str]:
        try:
            return page.evaluate(
                """(limit) => [...document.querySelectorAll('a[href]')]
    ...
    def crawl(self, url: str, options: CrawlOptions) -> CrawlResult:
        ...
        with sync_playwright() as p:
            ...
            for page_no in range(1, options.max_pages + 1):
                ...
                signature = tuple(sorted(clean_text(item.get("href")) for item in candidates if item.get("href")))
                ...
                for candidate in candidates:
                    ...
                    if job is None:
                        continue
                    if options.keyword and options.keyword.lower() not in f"{job.title} {job.extra}".lower():
                        continue
                ...
                if not next_action:
                    if len(jobs) == before:
                        exhausted = True
                        break
                    # Infinite-scroll style pages can grow without a next control.
                    continue
            ...
            if not jobs:
                ...
                warnings.append(f"QUALITY_FAIL no HCMCloud job-detail links found; body_sample={sample}")
                if hrefs:
                    warnings.append("HCMCloud href sample:\n" + "\n".join(hrefs))
            browser.close()

        values = list(jobs.values())[: options.max_jobs]
        if total_hint is not None and len(values) < min(total_hint, options.max_jobs):
            warnings.append(
